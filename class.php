...
class Comment
{
  ...
  //  コメントへの返信コメントを取得する
  function get_reply_comments($memo_id)
  {
    try {
      $dbh = db_connect();
      $sql = "SELECT *
            FROM comment
            WHERE memo_id = :id AND comment_id = :comment_id";
      $stmt = $dbh->prepare($sql);
      $stmt->execute(array(':id' => $memo_id, ':comment_id' => $this->id));
      return $stmt->fetchAll();
    } catch (\Exception $e) {
      error_log($e, 3, "../../php/error.log");
      _debug('返信コメント取得失敗');
